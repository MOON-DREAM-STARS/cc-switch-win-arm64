...
fn kept_previous(app: AgentUsageRebuildApp, result: SessionSyncResult) -> ProviderUsageRebuildResult {
            ...
            if root.join("state.db").is_file()
                || root
                    ...
                    && fs::read_dir(root.join("profiles"))
                        ...
                        .into_iter()
                        .flatten()
                        .any(|entry| entry.ok().is_some_and(|entry| entry.path().join("state.db").is_file()))
            {
                Ok(())
            } else {
                Err(AppError::Config(
...
fn ensure_readable_directory(path: &Path, label: &str) -> Result<(), AppError> {
    if !path.is_dir() {
        return Err(AppError::Config(format!("{label} 目录不存在: {}", path.display())));
    }
    fs::read_dir(path)
        .map(|_| ())
        .map_err(|error| AppError::Config(format!("无法读取 {label} 目录 {}: {error}", path.display())))
}
...
fn sync_provider_in_stage(
    ...
    match app {
        ...
        AgentUsageRebuildApp::Hermes => {
            let detailed = crate::services::session_usage_hermes::sync_hermes_usage_detailed(stage_db)?;
...
fn publish_provider_from_stage(
    ...
    conn.execute("ATTACH DATABASE ?1 AS rebuild_stage", [stage_path.to_string_lossy().as_ref()])
    ...
    let result = (|| {
        ...
        let tx = conn.unchecked_transaction().map_err(|error| {
            AppError::Database(format!("开启 provider 发布事务失败: {error}"))
        })?;
...
fn rebuild_non_codex_provider(
    ...
    drop(stage_db);
    finalize_staged_provider(db, &stage_path, app, cursor_scope, sync_result, identity_count)
}
...
fn finalize_staged_provider(
    ...
    let eligible = sync_result.errors.is_empty()
    ...
    if !eligible {
        ...
        if result.errors.is_empty() {
            result.errors.push(
                "provider 扫描未产生有效 session identity，保留上一代数据".to_string(),
            );
        }
    ...
    for app in apps {
        if let Some(Err(error)) = preflight.remove(&app) {
            providers.push(kept_previous(
                app,
                sync_error(format!("{} source preflight failed: {error}", app.app_type())),
            ));
            continue;
        }
...
mod tests {
    ...
    fn staged_publish_replaces_only_selected_provider_and_proxy_survives() -> Result<(), AppError> {
        ...
        let cursor_scope = CursorScope::PathPrefixes(vec![normalized_path(
    ...
    fn failed_or_deferred_stage_keeps_live_provider_rows_and_cursor() -> Result<(), AppError> {
        ...
        let cursor = claude_root.join("unchanged.jsonl").to_string_lossy().to_string();
        {
            let conn = lock_conn!(db.conn);
            conn.execute(
                "INSERT INTO proxy_request_logs (
