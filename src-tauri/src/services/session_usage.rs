...
fn discover_claude_identity(
    ...
    let mut metadata = SessionNodeMetadata::default();
    metadata.title = title;
    metadata.project_dir = project_dir;
    metadata.source_path = Some(path.to_string_lossy().into_owned());
    metadata.last_synced_at = SystemTime::now()
...
fn sync_single_file_with_context(
    ...
    for parsed in messages.values() {
        ...
        if msg
            .session_id
            .as_deref()
            .map_or(true, |session_id| session_id.trim().is_empty())
...
mod tests {
    ...
    fn claude_native_title_and_cwd_are_read_without_message_fallback() {
        ...
        assert_eq!(identity.claim.metadata.title.as_deref(), Some("Native title"));
        assert_eq!(
