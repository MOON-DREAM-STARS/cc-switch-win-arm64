...
fn load_hermes_sqlite_session_metadata(path: &Path) -> HashMap<String, HermesNativeMetadata> {
    ...
    let Some(id_column) = find_native_column(&columns, &["session_id", "sessionId", "id"])
    else {
        return metadata;
    };
    ...
    let sql = format!(
        ...
        quote_sql_identifier(id_column), title_expr, project_expr
    );
...
fn read_hermes_session_file_metadata(
    path: &Path,
) -> Option<(String, HermesNativeMetadata)> {
...
> {
    ...
    for row in &rows {
        ...
        let current = SnapshotCounters {
            ...
            selected_cost_usd: row.selected_cost_usd.clone(),
        ...
        let (next, delta) = match previous {
            None => {
                ...
                (
                    SnapshotCounters {
                        cost_baseline_usd: current.selected_cost_usd.clone(),
                        ..current.clone()
                    },
                    None,
                )
...
fn normalized_snapshot(
    ...
    NormalizedUsageSnapshot {
        ...
        correction_state: Some(format_cost_state(
            counters.cost_baseline_usd.clone(),
            counters.emitted_cost_balance_usd,
        )),
    }
}
...
fn build_fact(
    ...
    NormalizedUsageRollupFact {
        ...
        correction_state: delta.cost_usd.as_ref().map(|_| {
            format_cost_state(
                delta.cost_baseline_usd.clone(),
...
impl HermesSnapshotDelta {
    fn between(previous: &SnapshotCounters, current: &SnapshotCounters) -> Result<Self, AppError> {
        let next_cost_baseline = match (
            ...
            (None, Some(cost)) => Some(cost.clone()),
            (Some(baseline), _) => Some(baseline.clone()),
            (None, None) => None,
        };
        let target_balance = match (
            ...
            (Some(baseline), Some(cost)) => {
                let amount = cost.clone() - baseline.clone();
...
fn parse_source_row(row: &Row<'_>) -> Result<HermesSourceRow, AppError> {
    ...
    let (selected_cost_usd, selected_cost_kind) = select_cost(
        estimated_cost_usd.clone(),
        actual_cost_usd.clone(),
