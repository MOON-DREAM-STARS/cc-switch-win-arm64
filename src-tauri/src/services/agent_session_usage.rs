...
impl Default for UsagePrecision {
    fn default() -> Self {
        Self::Unavailable
    }
}
...
impl Default for TimeSemantics {
    fn default() -> Self {
        Self::Unavailable
    }
}
...
impl Default for RequestCountSemantics {
    fn default() -> Self {
        Self::Unavailable
    }
}
...
const CAPABILITY_REGISTRY: [AgentUsageCapability; 8] = [
...
fn query_usage_groups(
    ...
    let root_placeholders = std::iter::repeat("?")
        .take(root_ids.len())
        .collect::<Vec<_>>()
        .join(", ");
...
/// Return the immutable eight-App registry from the normalized contract.  No
